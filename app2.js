/* Hacer análisis de sorteos con varias figuras - pendiente	*/
...
(async function () {
	...
	try {
		...
		for (let s = 0; s < totalSorteos; s++) {
			...
			let cantidadPremios = 2;

			//Limpiar tachados y balotas de sorteo anterior
			p = await limpiaJuego();

			//Eleccion figura
			p = await agregarFigura(idFigura1, 1);
			p = await agregarFigura(idFigura2, 2);
			...
			for (let i = 0; i < 75; i++) {
				...
				if (balota_juegoObj.ganadores != null) {
					// break;
					cantidadPremios--;
					ganadoresAr.push(balota_juegoObj.ganadores.length);
					console.info("***ganadoresAr: " + ganadoresAr);

					console.info("***cantidadPremios: " + cantidadPremios);
		...
		console.info("\n----------------------------------------------------");
		console.info("Sorteos: "+totalSorteos+" | Figura:" + idFigura1 + ", " + idFigura2
			+ " | Total Ganadores:" + totalGanadores);
		console.info("----------------------------------------------------");
